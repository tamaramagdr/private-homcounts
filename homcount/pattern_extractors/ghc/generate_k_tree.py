...
def get_pattern_list(size, pattern_count, min_size=0, max_treewidth=10, seed=0):
...
def random_ktree_profile(graphs, size='max', max_treewidth=10, density=False, seed=8, pattern_count=50, early_stopping=10, metadata=None, min_embedding=True, add_small_patterns=False, pattern_file=None, filter_and_retry=True, **kwargs):
    ...
    if add_small_patterns:
        ...
        kt_list, td_list = get_pattern_list(size, pattern_count=pattern_count - 4, min_size=min_pattern_size,
                                            max_treewidth=max_treewidth, seed=seed)
    ...
    if filter_and_retry:
        ...
        while embeddings.shape[1] < pattern_count:
            kt_tmp, td_tmp = get_pattern_list(size, pattern_count=pattern_count - embeddings.shape[1],
                                              min_size=min_pattern_size, max_treewidth=max_treewidth, seed=seed)
